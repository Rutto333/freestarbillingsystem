...
switch ($action) {
    ...
    case 'package':
        ...
        // Get user's router ID - this is the key fix
        $user_router_id = $user['router_id'] ?? null; // Adjust field name as per your DB schema
        ...
        if (!empty($_SESSION['nux-router'])) {
        ...
        } else if (!empty($user_router_id)) {
            // Filter by user's assigned router
            $routers = ORM::for_table('tbl_routers')->where('id', $user_router_id)->find_many();
            $rs = [];
            foreach ($routers as $r) {
                $rs[] = $r['name'];
            }
            ...
            // No router assigned - show empty results or error
            $routers = [];
            $radius_pppoe = [];
            $radius_hotspot = [];
            $plans_pppoe = [];
