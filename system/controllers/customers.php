    ...
    default:
        ...
        if ($search != '') {
            $query = ORM::for_table('tbl_customers')
                ...
                ->where_raw(
                    "(tbl_customers.username LIKE :search
                    OR tbl_customers.fullname LIKE :search
                    OR tbl_customers.address LIKE :search
                    OR tbl_customers.phonenumber LIKE :search
                    OR tbl_customers.email LIKE :search)
                    AND tbl_customers.status = :filter",
            ...
            $query = ORM::for_table('tbl_customers')
                ...
                ->select('tbl_routers.name', 'router_name')
                ->left_outer_join('tbl_routers', array('tbl_customers.router_id', '=', 'tbl_routers.id'))
                ->where("tbl_customers.status", $filter);
        }

        if ($order == 'lastname') {
            $query->order_by_expr("SUBSTR(tbl_customers.fullname, INSTR(tbl_customers.fullname, ' ')) $orderby");
        } else {
            if ($orderby == 'asc') {
