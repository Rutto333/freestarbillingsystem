...
switch ($action) {
    ...
    case 'delete':
        ...
        // Check if plan uses Radius or Router
        $routerName = $d['routers'];
        $isRadius = $d['is_radius'] ?? 0;

        // If it's a Radius plan, delete locally only
        if ($isRadius == 1 || $routerName == 'radius') {
            $d->delete();
            r2(getUrl('services/hotspot'), 's', Lang::T('Radius plan deleted successfully.'));
            break;
        }
        ...
        if ($_app_stage != 'demo') {
            ...
            if (file_exists($dvc)) {
                ...
                if (class_exists($deviceClass)) {
                    try {
                        (new $deviceClass())->remove_plan($d);
                    } catch (Exception $e) {
                        // Handle API error gracefully
                        $d->delete();
                        r2(getUrl('services/hotspot'), 'w', Lang::T('Error removing plan from router: ') . $e->getMessage());
                        break;
                    }
                } else {
                    // Class not found
                    $d->delete();
                    r2(getUrl('services/hotspot'), 'w', Lang::T('Device class not found. Plan deleted locally.'));
                    break;
                }
            } else {
                // Device file not found
                $d->delete();
                r2(getUrl('services/hotspot'), 'w', Lang::T('Device file not found. Plan deleted locally.'));
                break;
            }
        }
        ...
        // If all successful
        $d->delete();
        r2(getUrl('services/hotspot'), 's', Lang::T('Plan deleted and synced successfully.'));
        break;
    ...
    case 'add-post':
        ...
        if ($msg == '') {
            ...
            if (!$offlineCheck) {
                r2(getUrl("services/hotspot"), "e", Lang::T("Router not found."));
                break;
            }
    ...
    case 'pppoe-add-post':
        ...
        if ($msg == '') {
            ...
            if ($offlineCheck->status === "Online") {
                ...
                r2(getUrl("services/pppoe"), "w", Lang::T("Router is offline. Please sync the plan when the router is online."));
            }
