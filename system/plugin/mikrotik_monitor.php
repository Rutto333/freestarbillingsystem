...
function mikrotik_monitor_get_resources()
{
    ...
    // ✅ 1. Router not found or disabled
    if (!$mikrotik) {
        _log("Router ID $router_id not found or disabled.");
        sendTelegram("Router ID $router_id not found or disabled.");
        echo "<div style='color:red;'>Router not found or disabled.</div>";
        return;
    }

    // ✅ 2. Router offline — stop completely
    if (isset($mikrotik['status']) && strtolower(trim($mikrotik['status'])) !== 'online') {
        _log("Router '{$mikrotik['name']}' is offline. Monitoring skipped.");
        sendTelegram("Router '{$mikrotik['name']}' is offline. Please sync when back online.");
        echo "<div style='color:red;'>Router is offline. Please sync when it’s back online.</div>";
        return; // ❌ stop here, do not connect
    }

    try {
        // ✅ 3. Proceed only if router is online
        ...
        // Build HTML output (unchanged)
        $table = '
<style>
    .column-card-container { display:flex; flex-wrap:wrap; justify-content:space-between; margin-top:20px; }
    .column-card { flex-basis:calc(33.33% - 20px); background-color:#fff; border:1px solid #ddd;
        border-radius:5px; box-shadow:0 0 10px rgba(0,0,0,0.1); margin-bottom:20px; }
    .column-card-header { background-color:#009879; color:#fff; padding:10px; border-radius:5px 5px 0 0; }
    .column-card-content { padding:15px; }
    .column-card-content table { width:100%; border-collapse:collapse; }
    .column-card-content th, .column-card-content td { padding:8px; border-bottom:1px solid #ddd; }
    .column-card-content th { text-align:left; background-color:#f2f2f2; }
    .column-card-content td { text-align:right; }
    @media only screen and (max-width:768px) { .column-card { flex-basis:calc(100% - 20px); } }
</style>
    ...
    <div class="column-card">
        ...
        <div class="column-card-content">
            <table><tbody>
                ...
                <tr><th>Mem used/free</th><td>' .
                    mikrotik_monitor_formatSize($res->getProperty('total-memory') - $res->getProperty('free-memory')) .
                    ' / ' . mikrotik_monitor_formatSize($res->getProperty('free-memory')) . '</td></tr>
            </tbody></table>
        </div>
    </div>
    ...
    <div class="column-card">
        ...
        <div class="column-card-content">
            <table><tbody>
                ...
                <tr><th>CPU count/freq/load</th><td>' .
                    $res->getProperty('cpu-count') . ' / ' .
                    $res->getProperty('cpu-frequency') . ' / ' .
                    $res->getProperty('cpu-load') . '</td></tr>
                <tr><th>Hdd</th><td>' .
                    mikrotik_monitor_formatSize($res->getProperty('free-hdd-space')) . ' / ' .
                    mikrotik_monitor_formatSize($res->getProperty('total-hdd-space')) . '</td></tr>
        ...
        _log('Mikrotik Monitor Error fetching system resources: ' . $e->getMessage());
        sendTelegram('Mikrotik Monitor Error fetching system resources: ' . $e->getMessage());
        echo "<div style='color:red;'>Failed to fetch router resources: " . htmlspecialchars($e->getMessage()) . "</div>";
    }
}
...
function mikrotik_monitor_get_interfaces_list()
{
    ...
    // ✅ Check router status before proceeding
    if (isset($mikrotik['status']) && strtolower($mikrotik['status']) !== 'online') {
        _log("Router '{$mikrotik['name']}' is offline. Skipping interface check.");
        sendTelegram("Router '{$mikrotik['name']}' is offline. Please sync when back online.");
        return [];
    }

    // Continue only if router is online
    $client = Mikrotik::getClient(
        $mikrotik['ip_address'],
        $mikrotik['username'],
        $mikrotik['password']
