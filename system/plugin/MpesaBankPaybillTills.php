...
function MpesaBankPaybillTills_validate_config()
{
  global $config;
  if (empty($config['Stkpaymenttype'])) {
    sendTelegram("Mpesa Bank Paybill Tills payment gateway not configured");
    r2(U . 'order/balance', 'w', Lang::T("Admin has not yet setup the payment gateway, please tell admin"));
  }
}

function MpesaBankPaybillTills_show_config()
{
  global $ui, $config;
  setupMpesaBanksPaybillsTillsTable();
  $banks = ORM::for_table('tbl_banks_paybills_tills')->find_many();
  $ui->assign('banks', $banks);
  $ui->assign('_title', 'Mpesa Bank Paybill Tills for STK Push ' . $config['CompanyName']);
  $ui->display('mpesabankpaybilltills.tpl');
}

function MpesaBankPaybillTills_save_config()
{
  global $admin, $_L;
  $paymenttype = _post('paymenttype');
  //CHECK IF THE PAYMENT TYPE IS BANK STK PUSH
  if ($paymenttype == "") {
    r2(U . 'paymentgateway/MpesaBankPaybillTills', 'e', 'Please select a payment type');
  }

  $d = ORM::for_table('tbl_appconfig')->where('setting', 'Stkpaymenttype')->find_one();
  if ($d) {
    $d->value = $paymenttype;
    $d->save();
  } else {
    $d = ORM::for_table('tbl_appconfig')->create();
    $d->setting = 'Stkpaymenttype';
    $d->value = $paymenttype;
    $d->save();
  }
  ...
  if ($paymenttype == "paybill") {
    ...
    $d = ORM::for_table('tbl_appconfig')->where('setting', 'Stkpaybill')->find_one();
    if ($d) {
      $d->value = $paybill_number;
      $d->save();
    } else {
      $d = ORM::for_table('tbl_appconfig')->create();
      $d->setting = 'Stkpaybill';
      $d->value = $paybill_number;
      $d->save();
    }
  }
  ...
  if ($paymenttype == "till") {
    ...
    $d = ORM::for_table('tbl_appconfig')->where('setting', 'Stktill')->find_one();
    if ($d) {
      $d->value = $till_number;
      $d->save();
    } else {
  ...
  if ($paymenttype == "bank") {
    $bank_paybill_number = _post('bank_paybill_number');
    $bank_account_number = _post('bank_account_number');

    $d = ORM::for_table('tbl_appconfig')->where('setting', 'stk_bank_paybill_number')->find_one();
    if ($d) {
      $d->value = $bank_paybill_number;
      $d->save();
    } else {
      $d = ORM::for_table('tbl_appconfig')->create();
      $d->setting = 'stk_bank_paybill_number';
      $d->value = $bank_paybill_number;
      $d->save();
    }

    $d = ORM::for_table('tbl_appconfig')->where('setting', 'stk_mpesa_bank_account_number')->find_one();
    if ($d) {
      $d->value = $bank_account_number;
      $d->save();
    } else {
      $d = ORM::for_table('tbl_appconfig')->create();
      $d->setting = 'stk_mpesa_bank_account_number';
      $d->value = $bank_account_number;
      $d->save();
    }
  }
  ...
  _log('[' . $admin['username'] . ']: Mpesa Bank Paybill Tills for STK Push details ' . $_L['Settings_Saved_Successfully'], 'Admin', $admin['id']);

  r2(U . 'paymentgateway/MpesaBankPaybillTills', 's', $_L['Settings_Saved_Successfully'] . ' for Mpesa Bank Paybill Tills for STK Push');
}


function MpesaBankPaybillTills_create_transaction($trx, $user)
{
...
function setupMpesaBanksPaybillsTillsTable($tableName = 'tbl_banks_paybills_tills')
{
  $db = ORM::get_db();
...
function MpesaBankPaybillTills_payment_notification()
{
